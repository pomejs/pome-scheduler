...
function decoderTest(){
  ...
  result.push(CronTrigger.createTrigger('12    2 3,4,5 4 5 1').trigger);
  
  result.push(CronTrigger.createTrigger('*    1-3,2-9,4 3,4,5 4-9 5 1').trigger);
  
  result.push(CronTrigger.createTrigger('12    2 3 4 5 1').trigger);
  console.log(result);
}

function nextTimeTest(count){
  var cronTrigger = CronTrigger.createTrigger('0 0 0 1 0 2-5');
  var timer = cronTrigger.trigger;
 ...
 for(var i = 0; i < count; i++)
   r1 = cronTrigger.nextExcuteTime(value);
 var end = Date.now();
